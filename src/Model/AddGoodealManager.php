...
class AddGoodealManager extends AbstractManager
{
    ...
    public function insert(array $announcement): void
    {
        ...
        $this->pdo->prepare("SELECT id FROM region WHERE region_name=:region");
        $statement->bindValue(':region', $announcement['region'], PDO::PARAM_STR);  
        $region_id = $statement->fetchAll();

        $this->pdo->prepare("SELECT id FROM author WHERE firstname=:firstname AND lastname=:lastname AND email=:email");
        ...
        $author_id = $statement->fetchAll();



        $statement = $this->pdo->prepare("INSERT INTO announcement (region_id, title, message, adress, author_id, category, date, date_start, date_end, image, city, zipcode )
        VALUES (:region_id, :deal-name, :description, :adress, :author_id, :category, :date, :start-date, :end-date, :avatar, :city :zip-code)");
